  ...
  $(function(){    
	...
	$(".help-block").hide();
		if(((/\d/).test($("input[name=userName]").val()))||($("input[name=userName]").val().length<=5)||($("input[name=userName]").val().length>=12)){
			$("#accountvalid").show(); //check for non-digits
		}
		else if(((/\d/).test($("input[name=customerName]").val()))||(!$("input[name=customerName]").val())||((/[!@#$%^&*()_+\-=\[\]{};':"\\|,<>\/?]/).test($("input[name=customerName]").val()))){
			$("#namevalid").show();
			//$("#customerName").focus();
  ...
  $(function(){    
	...
	$(".help-block").hide();
		if($("#branchCode").val()== 0){
			$("#branchvalid").show();
		}else if(((/\D/).test($("input[name=loanAmount]").val())==true)||(($("input[name=loanAmount]").val())==false)){
			$("#amountvalid").show();
		}else if(((/\D/).test($("input[name=repayPeriod]").val())==true)||(($("input[name=repayPeriod]").val())==false)){
			$("#periodvalid").show();
		}
		else{
			$("#panel1").hide();
